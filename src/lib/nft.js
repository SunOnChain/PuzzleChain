...
/// Until then, minting is disabled rather than failing confusingly mid-flow.
...
/// Each puzzle's image only ever needs to be pinned once — every solver's NFT
/// metadata can point at the same Image CID. Cached in localStorage so repeat
/// mints (by anyone, for this puzzle) skip the re-upload entirely.
...
/// Marketplace-compatible NFT metadata (OpenSea-style `attributes` array),
/// plus the same fields duplicated as plain top-level keys for easy reading
/// by anything that doesn't bother parsing `attributes`.
export function buildAchievementMetadata({ puzzle, pieces, secs, address, imageCid }) {
  ...
  return {
    ...
    // Plain duplicates of the above, for convenience:
    puzzleId: puzzle.id,
...
/// Full mint flow:
///   solved → build metadata → upload metadata to Pinata → safeMint() → wallet
///   confirmation → tx confirmation → return everything the caller needs to
///   persist (txHash, tokenId, tokenURI, imageCid).
export async function mintAchievement({ puzzle, pieces, secs, address }) {
  ...
  if (!window.ethereum) throw new Error("No wallet found. Install MetaMask to mint.");
  ...
  for (const log of receipt.logs || []) {
    ...
    } catch { /* not one of our events — ignore */ }
  }
